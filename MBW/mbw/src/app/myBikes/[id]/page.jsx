...
export default function BikeDetailPage() {
  ...
  useEffect(() => {
    const fetchBike = async () => {
      try {
        const res = await axios.get(`/api/bikes/myBikes/${id}`, {
  ...
  const handleDelete = async () => {
    try {
      setDeleting(true);
      setActionError("");
      await axios.delete(`/api/bikes/myBikes/${id}?cascade=1`, {
        withCredentials: true,
      });
      router.push("/myBikes?deleted=1");
    } catch (err) {
  ...
  if (!bike) {
    return (
      <div className="p-6 max-w-xl mx-auto">
        ...
        <Link href="/myBikes" className="text-blue-600 underline mt-4 block">
          Back to My Bikes
        </Link>
      </div>
    );
