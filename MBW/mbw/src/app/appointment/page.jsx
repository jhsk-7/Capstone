...
export default function AppointmentPage() {
  ...
  useEffect(() => {
    const fetchUserAndBikes = async () => {
      try {
        // User
        const userRes = await axios.get("/api/users/myProfile", { withCredentials: true });
        const user = userRes.data.data;
        setUserId(user._id);

        // Bikes
        const bikesRes = await axios.get("/api/bikes/myBikes", { withCredentials: true });
        setBikes(bikesRes.data.data || []);

        // Services
        const servicesRes = await axios.get("/api/service/services", { withCredentials: true });
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post("/api/service/appointment", payload, { withCredentials: true });
  ...
  return (
    <>
      ...
      <div className="fixed left-4 top-24 z-40">
        <Link
          href="/appointment/myAppointments"
