...
export default function SignupPage() {
  ...
  const onSignup = async (e) => {
    ...
    try {
      setErrMsg("");
      setLoading(true);
      const res = await axios.post("/api/users/userAuth/signup", user, { withCredentials: true });

      router.push("/user/addBike");
    } catch (error) {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      <div className="mt-4 text-sm text-gray-700">
        ...
        <Link href="/user/auth/login" className="text-blue-100 underline hover:text-white">
          Log in
        </Link>
      </div>
    </div>
